...
def identity_block_test(target):
    ...
    A3 = target(X, f=2, filters=[4, 4, 3], initializer=lambda seed=0: constant(value=1))
    ...
    tf.random.set_seed(2)
    A4 = target(
        X, f=3, filters=[3, 3, 3], initializer=lambda seed=7: constant(value=1), training=True
    )
    A4np = A4.numpy()
...
def convolutional_block_test(target):
    ...
    convolutional_block_output1 = [
        [
            [[0.0, 0.77726924, 0.0, 1.499777, 0.0, 0.0], [0.0, 1.026826, 0.0, 1.2753085, 0.0, 0.0]],
            [[0.0, 1.0379409, 0.0, 1.665507, 0.0, 0.0], [0.0, 1.0401409, 0.0, 1.3495028, 0.0, 0.0]],
        ],
        [
            [
                [0.0, 2.3318284, 0.0, 4.4993644, 0.0, 0.0],
                [0.0, 3.0805848, 0.0, 3.8257396, 0.0, 0.0],
            ],
            [[0.0, 3.1137722, 0.0, 4.9962516, 0.0, 0.0], [0.0, 3.120653, 0.0, 4.048527, 0.0, 0.0]],
        ],
    ]
    ...
    assert tuple(tf.shape(A).numpy()) == (
        3,
        1,
        1,
        6,
    ), "Wrong shape. Make sure you are using the stride values as expected."
    ...
    C = target(X, f=2, filters=[2, 4, 6], training=True)
    # assert np.allclose(
    #     C.numpy(), convolutional_block_output2, atol=1e-2
    # ), "Wrong values when training=True."
    print(C)
    print(convolutional_block_output2)
