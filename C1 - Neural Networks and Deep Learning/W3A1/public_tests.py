...
def initialize_parameters_test(target):
    ...
    expected_output = {
        ...
        "W2": np.array(
            [
                [-5.96159700e-03, -1.91304965e-04, 1.17500122e-02, -7.47870949e-03, 9.02525097e-05],
                [
                    -8.78107893e-03,
                    -1.56434170e-03,
                    2.56570452e-03,
...
def forward_propagation_test(target):
    ...
    expected_cache = {"Z1": expected_Z1, "A1": expected_A1, "Z2": expected_Z2, "A2": expected_A2}
...
def compute_cost_test(target):
    ...
    ), f"Wrong value. Expected: {expected_output_1} got: {output_1}"

    assert type(output_2) == float, "Wrong type. Float expected"
    assert np.isclose(
        output_2, expected_output_2
    ), f"Wrong value. Expected: {expected_output_2} got: {output_2}"
...
def update_parameters_test(target):
    ...
    expected_output = {"W1": expected_W1, "b1": expected_b1, "W2": expected_W2, "b2": expected_b2}
...
def nn_model_test(target):
    ...
    expected_output = {
        ...
        "b1": np.array([[0.00285386], [0.01642488], [-0.01114986], [-0.01656405], [0.01042274]]),
        "W2": np.array([[-1.45325879, -2.65768451, 2.26286606, 2.97274518, -2.18860534]]),
        "b2": np.array([[0.00758617]]),
    }
